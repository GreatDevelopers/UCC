...
class AvatarCropForm(forms.Form):
    ...
    def clean(self):
        if int(self.cleaned_data.get('right')) - int(self.cleaned_data.get('left')) < 150:
            raise forms.ValidationError(_("You must select a portion of the image with a minimum of 150x150 pixels."))
        else:
            return self.cleaned_data
